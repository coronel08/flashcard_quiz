...
class QuizQuestionsFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):  # return all the objects that fit param that we set
        if self.value():
            quiz_title = self.value()
            return queryset.filter(Q(quiz_title=quiz_title))
...
class AnswerQuestionFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):  # create a clickable links on right hand side
        quizzes = models.Quiz.objects.all()
        lookups = ()
        for quiz in quizzes:
            lookups += ((quiz.title, quiz.title),)
    ...
    def queryset(self, request, queryset):  # return all objects that fit parameter that we set
        if self.value():
            quiz_title = self.value()
